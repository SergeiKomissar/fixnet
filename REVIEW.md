    ...
    if [ "$tcp_base" -eq 0 ]; then
        ...
        if [ "$allow_tested" -ge 1 ] && [ "$allow_open" -ge 1 ]; then
            if [ "$allow_open" -ge 2 ]; then
                ...
                echo -e "  Вывод: ${Y}Возможна частичная доступность (открылся лишь один контрольный — слабый признак).${N}"
            fi
            echo -e "  ${D}Контрольные ресурсы: внешние (1.1.1.1 / apple.com) — нет; региональные:${_basis}${N}"
            ...
            if [ "${VPN_ACTIVE:-0}" -eq 1 ]; then
                ...
                echo -e "  ${D}Возможен режим частичной доступности / белого списка или фильтрация сети (возможна, не доказано).${N}"
            fi
        else
