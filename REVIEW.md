...
# Формулировки осторожные: «сеть проходит» (не «доступен»), «региональный отказ»
...
ai_report() {
    ...
    echo -e "  ${D}«Сеть проходит» = API-домен ответил без явных признаков регионального отказа.${N}"
