...
probe_report() {
    ...
    if [ "$tcp_base" -eq 0 ]; then
        # Внешние (1.1.1.1, apple) закрыты. РАЗВИЛКА: общий обрыв vs «белый список» —
        # проверим локальные/разрешённые ресурсы. Если они открываются, а внешние нет →
        # whitelist_like (частичная доступность). ТОЛЬКО когда факты подходят; «возможна».
        local loc_open=0 loc_tested=0
        for _h in ya.ru vk.com; do
            local _r; _r=$(probe_tcp "$_h" 443)
            [ "$_r" = "skip" ] && continue
            loc_tested=$((loc_tested+1))
            [ "$_r" = "open" ] && loc_open=$((loc_open+1))
        done
        if [ "$loc_tested" -ge 1 ] && [ "$loc_open" -ge 1 ]; then
            echo -e "  Вывод: ${Y}Похоже на частичную доступность (режим «белого списка»).${N}"
            echo -e "  ${D}Внешние ресурсы (1.1.1.1 / apple.com) не открываются, а локальные (ya.ru / vk.com) — да."
            echo -e "  Это не похоже на поломку Wi-Fi. Возможна фильтрация сети или белый список (возможна, не доказано).${N}"
        else
