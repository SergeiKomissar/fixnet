# Ревью `netinfo.sh` — задание для эксперта

> Документ самодостаточный: ниже сначала задание, затем **полный снимок кода**.
> Канонический исходник — `netinfo.sh` в этом репозитории; код в конце файла мог
> чуть устареть относительно репозитория, сверяйтесь с ним.
>
> **Раунд 2.** По итогам первого круга ревью (Manus + Gemini) уже внедрено:
> строгие коды HTTPS + явный captive-portal; детект VPN по маршруту до внешнего
> IP (ловит split-default); geo-фолбэк строго по HTTPS (`ifconfig.co`, код
> страны); `json_get` через python3; джиттер (stddev) и потери пакетов из одного
> `ping`; порог utun ≤4 = норма; санитизация SSID/geo от ANSI-инъекций.
> Сознательно НЕ делали: мониторинг/демон, сравнение с прошлым замером, DNS-leak
> (нельзя без догадок). Под вопросом на будущее: `--log`, IPv6-leak, параллелизация.

## Контекст

`netinfo.sh` — **read-only** утилита диагностики сети на macOS (Apple Silicon, M4,
русифицированная система), companion к «ремонтнику» `fixnet.sh`. Разделение ролей
жёсткое: `fixnet` чинит (root, меняет маршруты/DNS/Wi-Fi), `netinfo` только
**смотрит и объясняет**, ничего не меняя.

Целевой пользователь — **владелец машины, не сисадмин**. Запускается кнопкой
`netinfo.command` на рабочем столе через `sudo` (root нужен для RSSI и для флага
скрытой сети). Три уровня вывода:

- **обычный** — человеческий язык без жаргона: крупный вердикт + диагноз + один совет;
- **`--tech`** — технические детали (интерфейсы, маршруты, utun, RSSI/SNR, RTT min/avg/max, DNS-серверы);
- **`--advice`** — контекстные советы по текущей сети.

Внутри окна работает меню: `t` (детали), `a` (советы), `r` (обновить замер), `Enter` (закрыть).

Боль, которую утилита лечит: при работе через слабый/далёкий VPN в чужой сети
непонятно — есть ли интернет, через какую страну выход, не потёк ли реальный IP,
и где узкое место (Wi-Fi / провайдер / VPN).

## Философия проекта (нельзя нарушать — проверьте соответствие кода)

1. **Точность важнее всего, но НЕ гадать.** Если надёжного источника данных нет —
   молчим, а не показываем наугад. Так сделано со «скрытой» сетью: флаг берётся из
   `known-networks.plist` (только root) и только для СОХРАНЁННЫХ сетей; для
   незапомненной сети надёжного источника нет — поэтому не показываем вовсе.
2. **Read-only.** Скрипт НИЧЕГО не меняет в системе. Подтвердите, что ни одна ветка
   не мутирует состояние (нет `route`, `ipconfig set`, `networksetup -set...`,
   `killall` и т.п. — только чтение).
3. **Один сбор → много рендеров.** Все измерения (ping/speed/geo/RSSI) делаются
   ОДИН раз в `collect_all`; `t`/`a` только перерисовывают уже собранное. Иначе на
   соседних экранах прыгали бы цифры и терялось бы доверие. Пересобирает только `r`.

## На что обратить особое внимание

1. **Надёжность парсинга системных утилит** (`wdutil info`, `ipconfig getsummary`,
   `system_profiler SPAirPortDataType`, `scutil --dns`, `ping`, `route`,
   `networksetup`) — устойчивость к разным версиям macOS и к локали. Где парсинг
   хрупкий; где сбой даст молча пустое значение, а где — ложный вывод.
2. **Корректность синтеза статусов** `MAIN_OK / QLEVEL / WORK / SEC_OPEN /
   VPN_ACTIVE`: логические дыры, недостижимые ветки, адекватность порогов
   (RTT 80/180 мс, разброс 50/100 мс, скорость 8/15 Мбит/с, RSSI −60/−67 dBm,
   utun 2/6).
3. **Граничные случаи**: нет интернета; ICMP зарезан, но HTTPS жив; DNS жив для
   `curl`, но `dig` падает; `--no-geo` / `--no-speed`; запуск без root; отсутствие
   `curl` / `python3` / `dig`; кабель/USB-раздача вместо Wi-Fi; пустой или
   юникодный SSID; запуск в пайпе/cron (не должно зависать на меню).
4. **Безопасность и приватность**: внешние запросы (ipinfo/ip-api — фолбэк по
   HTTP; Cloudflare speed); отсутствие инъекций при подстановке SSID и полей JSON
   в команды; поведение под `set -u`; корректность `read -r ... <<<`.
5. **Архитектура `collect_all` → `render_*`**: глобальные переменные без `local`,
   идемпотентность повторного `collect_all` (клавиша `r`), детект TTY
   (`[ -t 0 ] && [ -t 1 ]`) для меню.
6. **Дубль хелперов с `fixnet.sh`** (`detect_iface`, `check_dns`) — риск рассинхрона
   при правках; стоит ли выносить в общий источник.
7. **Фаза 5 — `--probe` (проба TCP/UDP-выхода для VPN), фундамент честности.**
   Проверьте именно границу доказуемого: TCP-connect = факт (open/closed), UDP =
   НЕ факт (нет ответа ≠ блок). UDP меряется только через рефлекторы, обязанные
   ответить (`dig` 53, `sntp` 123, STUN 3478→19302 через python3, ≤2 c/якорь).
   Весовая логика: STUN ответил ИЛИ ≥2 из {DNS,NTP,STUN} → «полной блокировки не
   видно»; только DNS/только NTP → «часть UDP»; всё молчит при живом TCP → «UDP не
   отвечает». Ищем: не проскочило ли где-то самоуверенное «порт заблокирован» /
   «DPI доказан»; корректность развязки «IP открыт, домен нет → DNS, не сеть»
   (два TCP-якоря 1.1.1.1 и www.apple.com); что НЕ стучим пустыми UDP в
   1194/51820/500/4500. `--probe` read-only, fixnet не участвует.
8. **Фаза 6 — link awareness + `--scan` + смысловой движок.** ИНВАРИАНТЫ (не нарушать):
   (а) «включи VPN» советуем ТОЛЬКО когда `VPN_ACTIVE=0` — WORK ветвится по VPN_ACTIVE
   И SEC_OPEN (был баг: открытая+VPN-активен → «включи VPN», хотя VPN уже активен);
   (б) 5G/LTE НЕ определяем (Mac не видит поколение сотовой — проверьте, что нигде не
   выводится как факт; при раздаче только подсказка «смотри на iPhone»); (в) раздача
   с телефона по подсети `172.20.10.x` + сервисам `iPhone/iPad USB`; «Подключение» =
   физический линк даже при активном VPN (не «через utun»); (г) `--scan` read-only,
   сам не подключается; сигнал соседей «—» где macOS не отдала (airport мёртв →
   `system_profiler`); открытая «только при Security: None», нет строки → «?» не
   «открытая»; (д) VPN-выход не пугает «отвалился» (ожидаемую страну не знаем).
9. **Фаза 6.1 — `--json` (контракт API).** JSON только ФАКТЫ и КОДЫ состояний, без
   человеческих фраз. Проверьте: валидность (`--json | python3 -m json.tool`); стойкость
   к странным SSID (пробелы/кавычки/emoji/ANSI/юникод/пустой — всё через python3
   `json.dumps`); `null` для недоступного (rssi без root, скорость при --no-speed);
   `provider` — сырой факт, не `short_org`; стабильность имён полей (это API).

## Что хочу получить от эксперта

**(А) Список реальных багов / хрупких мест** с приоритетом (критично / важно / косметика).

**(Б) Предложения по расширению функционала**, которые повысят диагностическую
ценность для сценария «слабый/далёкий VPN в чужой сети», но **не нарушат read-only
и принцип “не гадать”**. Для каждого — вердикт принять/отклонить с обоснованием.
Направления для оценки (не исчерпывающие):

- джиттер / packet-loss как отдельная метрика (сейчас есть только разброс RTT);
- авто-перепроверка через N секунд / режим мониторинга;
- история замеров (`--log` в файл) для разбора повторяющихся обрывов;
- явная проверка утечки DNS / IPv6 при включённом VPN;
- параллелизация замеров (geo + speed + ping) ради скорости отклика;
- явное определение captive portal (а не только «HTTPS не открывается»);
- сравнение «сейчас vs прошлый замер» в одном окне.

## Как запускать при проверке

```
sudo ./netinfo.sh          # точный осмотр (RSSI + скрытая сеть)
./netinfo.sh               # без root (без RSSI и флага «скрытая»)
./netinfo.sh --tech        # технические детали
./netinfo.sh --advice      # советы
./netinfo.sh --mtu         # измерить path MTU (DF-зонд) — read-only
./netinfo.sh --probe       # проба TCP/UDP-выхода для VPN (Фаза 5) — read-only
./netinfo.sh --scan        # соседние Wi-Fi сети (Фаза 6) — read-only
./netinfo.sh --json        # машиночитаемый контракт (Фаза 6.1)
./netinfo.sh --no-speed    # без замера скорости (платный трафик на раздаче)
./netinfo.sh --no-geo      # без обращения к geo-сервису
```

---

## Полный код (снимок `netinfo.sh`)
...
to_mbps() { awk -v b="$1" 'BEGIN{ v=b*8/1000000; if (v>=0.5) printf "%.0f", v; else print "" }'; }
...
if [ "$MAIN_OK" -eq 1 ]; then
    # Хвост про качество канала (общий для всех веток). Пусто при green. При красном
    # ОТДЕЛЬНО про потери пакетов: они рвут соединения (интерактив), формулировка иная.
    QSUFFIX=""
    if   [ "$QLEVEL" = yellow ]; then
        QSUFFIX=", но канал нестабильный — видеосвязь, удалённый рабочий стол и VPN-работа могут дёргаться"
    elif [ "$QLEVEL" = red ]; then
        if [ "${LOSS:-0}" -ge 10 ]; then
            QSUFFIX=", но есть потери пакетов — видеосвязь, удалённый рабочий стол и VPN-работа могут рваться"
        else
            QSUFFIX=", но канал плохой — видеосвязь, удалённый рабочий стол и большие загрузки будут страдать"
        fi
    fi
...
[ "${SSID_HIDDEN:-0}" -eq 1 ] && echo -e "  ${D}                    имя скрыто macOS (приватность Wi-Fi у Терминала; вернуть из CLI обычно нельзя — на диагностику не влияет)${N}"
...
if [ "$WANT_SPEED" -eq 1 ] && [ -z "$SPEED_ERR" ]; then
    ...
    [ -n "$UL_M" ] && echo -e "     отдача:          ${G}${UL_M} Мбит/с${N} ${D}— $(speed_word "$UL_M")${N}" \
                   || echo -e "     отдача:          ${Y}замер не удался${N}"
    # При fallback число несравнимо с замером через VPN/Cloudflare (другой сервер и маршрут).
    { [ -n "${SPEED_SRC:-}" ] && [ "${SPEED_SRC}" != cloudflare ]; } && \
        echo -e "     ${D}↳ замер через запасной сервер — грубая оценка, не для точного сравнения с VPN/Cloudflare${N}"
fi
...
# звучащего как security-leak: это просто следы VPN/Network Extension/Private Relay.
...
probe_report() {
    ...
    if [ "$tcp_base" -eq 0 ]; then
        ...
        echo -e "  Вывод: ${Y}TCP до внешнего IP (1.1.1.1) есть, но доменные проверки не проходят.${N}"
        echo -e "  ${D}Возможна проблема DNS, фильтрация имён или перехват DNS (DNS-like); механизм не доказан.${N}"
        echo -e "  ${D}Попробуй DoH / другой DNS (sudo fixnet --dns) или VPN.${N}"
    elif [ "$tls_cf" = "tcp_only" ] && [ "$tls_gh" = "tcp_only" ]; then
        CLASS="tls_sni_filter_like"
        echo -e "  Вывод: ${Y}TCP устанавливается, но TLS-соединение рвётся на нескольких доменах (TLS/SNI-похоже).${N}"
        echo -e "  ${D}Возможна TLS/SNI-фильтрация, TLS-инспекция, корпоративный прокси, проблема маршрута/MTU/VPN${N}"
        echo -e "  ${D}или сбой на стороне сети. Механизм не доказан (по одному клиентскому тесту DPI не доказать).${N}"
        echo -e "  ${D}Если включён VPN или сеть мобильная — проверь MTU: netinfo --mtu.${N}"
    else
...
render_explain() {
    ...
    echo -e "  ${C}Имя сайта в TLS (SNI)${N}"
    echo -e "  ${D}  В начале защищённого соединения имя сайта какое-то время видно открытым (SNI).${N}"
    echo -e "  ${D}  Некоторые сетевые фильтры обрывают соединение именно по нему — ещё до «замочка».${N}"
    echo -e "  ${C}Блокировка по адресу (L3) и по содержимому (L7)${N}"
    echo -e "  ${D}  L3 — режут сам IP сервера (не доходит даже TCP). L7 — соединение пропускают, но${N}"
    echo -e "  ${D}  фильтруют глубже (по имени сайта/протоколу): для тебя это «TLS рвётся» при живой сети.${N}"
    echo -e "  ${C}Частичная доступность («белый список»)${N}"
    echo -e "  ${D}  Часть ресурсов открывается, другие (особенно внешние) — нет. Это не поломка Wi-Fi,${N}"
    echo -e "  ${D}  а похоже на фильтрацию сети. netinfo пишет «возможна», не «точно» — по одному тесту${N}"
    echo -e "  ${D}  это не доказать.${N}"
    echo -e "  ${C}Фильтрация протокола VPN${N}"
    echo -e "  ${D}  Фильтр может реагировать не на IP, а на признаки протокола (OpenVPN/WireGuard). Если${N}"
    echo -e "  ${D}  VPN не подключается — иногда помогает другой протокол: IKEv2 / TCP 443 / режим маскировки.${N}"
    echo
...
EOF
    echo
    if [ "$redn" -eq 1 ]; then
        echo -e "  ${Y}Имена сетей скрыты macOS${N} ${D}(приватность Wi-Fi: ОС не отдаёт SSID приложениям из CLI/Терминала).${N}"
        echo -e "  ${D}Диапазоны/каналы/сигнал — реальные; имя — косметика. Вернуть имена из обычного Терминала, как${N}"
        echo -e "  ${D}правило, нельзя (Терминалу не выдаётся Location-доступ) — это ограничение Apple, не баг инструмента.${N}"
        echo
    fi
...
# (без политназваний источника блокировки — по телу видим лишь отказ сервиса/CDN/политики).
...
# отказа, НЕ «причину блокировки»; без политлексики (инвариант Фазы 14.1, см. CLAUDE.md).
why_report() {
    local url="$1"
    ...
    case "$wclass" in
        ...
        tls_blocked)
            ...
            echo -e "  ${D}Возможна TLS/SNI-фильтрация, корпоративный прокси, сбой сертификата или проблема сервера.${N}"
            echo -e "  ${D}${vpnhint}.${N}" ;;
        browser_challenge)
        ...
        http_forbidden)
            ...
            case "$wreason" in
                ...
                http_451)
                    echo -e "  ${D}Сервер сообщает об ограничении доступа по юридическим или политическим причинам (HTTP 451).${N}"
                    echo -e "  ${D}Попробуй другой сервер VPN и повтори, если ресурс должен быть доступен.${N}" ;;
        ...
        slow_or_timeout)
            ...
            if why_is_file "$url"; then
                # Мост «файл виснет → MTU» (Фаза 17.3): сайты открываются, а крупный файл
                # замирает — классика PMTU-blackhole; человек сам эту связь не увидит.
                echo -e "  ${D}Похоже, это ФАЙЛ. Если сайты открываются, а крупные файлы виснут — частый признак${N}"
                echo -e "  ${D}MTU-проблемы: проверь ${C}netinfo --mtu${N}${D} (чинит sudo fixnet --mtu).${N}"
                echo -e "  ${D}Скачать с докачкой при обрывах: ${C}access --get URL${N}${D}.${N}"
            fi ;;
    esac
    case "$wclass" in
